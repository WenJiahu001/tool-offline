...
import { ref } from 'vue'
import { Copy, Check, ArrowRightLeft, RotateCcw, Link, Unlink, Upload, Download, Keyboard } from 'lucide-vue-next'
  ...
  <div class="px-3 py-4 max-w-[98%] xl:max-w-[95%] mx-auto">
    ...
    <div class="mb-3.5 flex items-center justify-between gap-4">
      <div class="flex items-baseline gap-3">
        <h1 class="text-xl font-bold text-gray-900">URL 编解码工具</h1>
        <p class="text-xs text-gray-400 hidden sm:inline">本地 URL 编码、解码与查询参数解析，数据安全不上传</p>
      </div>
      <button 
    ...
    <div class="flex gap-2 mb-3">
      ...
      >
        <Link class="w-3.5 h-3.5 inline-block mr-1.5 -mt-0.5" />
        编码
      </button>
      <button
      ...
      >
        <Unlink class="w-3.5 h-3.5 inline-block mr-1.5 -mt-0.5" />
        解码
      </button>
      <button
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 inline-block mr-1.5 -mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- 编码/解码模式 -->
    <div v-if="activeMode === 'encode' || activeMode === 'decode'" class="space-y-3">
      <!-- 工具栏 -->
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-2 px-3">
        <div class="flex flex-wrap items-center gap-2">
          <button
            ...
            @click="encodeUrl"
            class="px-2.5 py-1.5 bg-teal-600 hover:bg-teal-700 text-white text-xs font-medium rounded-lg transition-colors flex items-center gap-1.5 shadow-sm"
          >
          ...
          <!-- 编码范围选择（仅编码模式） -->
          <div v-if="activeMode === 'encode'" class="flex items-center gap-3 text-xs text-gray-600">
      ...
      <!-- 编码范围说明 -->
      <div v-if="activeMode === 'encode'" class="bg-teal-50 rounded-lg p-2.5 border border-teal-100 text-xs text-teal-700">
      ...
      >
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <textarea
            ...
            class="w-full h-[calc(100vh-250px)] min-h-[500px] p-4 font-mono text-sm resize-none focus:outline-none"
            spellcheck="false"
          ></textarea>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <textarea
            ...
            class="w-full h-[calc(100vh-250px)] min-h-[500px] p-4 font-mono text-sm resize-none focus:outline-none bg-gray-50/50"
            spellcheck="false"
          ></textarea>
        </div>
      </div>
    ...
    <!-- URL 解析模式 -->
    <div v-if="activeMode === 'parse'" class="space-y-3">
      <!-- 工具栏 -->
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-2 px-3">
        <div class="flex items-center gap-2">
          <button
      ...
      <div v-if="parseResult" class="space-y-3">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div class="divide-y divide-gray-100">
            <div class="px-4 py-2 flex items-center gap-4 hover:bg-gray-50/50">
              <span class="text-xs text-gray-500 w-20 shrink-0">协议</span>
              <span class="font-mono text-xs text-gray-900 bg-blue-50 px-2 py-0.5 rounded">{{ parseResult.protocol }}</span>
            </div>
            <div class="px-4 py-2 flex items-center gap-4 hover:bg-gray-50/50">
              <span class="text-xs text-gray-500 w-20 shrink-0">主机名</span>
              <span class="font-mono text-xs text-gray-900">{{ parseResult.hostname }}</span>
            </div>
            <div class="px-4 py-2 flex items-center gap-4 hover:bg-gray-50/50">
              <span class="text-xs text-gray-500 w-20 shrink-0">端口</span>
              <span class="font-mono text-xs text-gray-900">{{ parseResult.port }}</span>
            </div>
            <div class="px-4 py-2 flex items-center gap-4 hover:bg-gray-50/50">
              <span class="text-xs text-gray-500 w-20 shrink-0">路径</span>
              <span class="font-mono text-xs text-gray-900 bg-purple-50 px-2 py-0.5 rounded">{{ parseResult.pathname }}</span>
            </div>
            <div class="px-4 py-2 flex items-center gap-4 hover:bg-gray-50/50">
              <span class="text-xs text-gray-500 w-20 shrink-0">锚点</span>
              <span class="font-mono text-xs text-gray-900 bg-orange-50 px-2 py-0.5 rounded">{{ parseResult.hash }}</span>
            </div>
            <div class="px-4 py-2 flex items-center gap-4 hover:bg-gray-50/50">
              <span class="text-xs text-gray-500 w-20 shrink-0">Origin</span>
              <span class="font-mono text-xs text-gray-900">{{ parseResult.origin }}</span>
            </div>
            <div class="px-4 py-2 flex items-center gap-4 hover:bg-gray-50/50">
              <span class="text-xs text-gray-500 w-20 shrink-0">查询字符串</span>
              <span class="font-mono text-xs text-gray-900 bg-green-50 px-2 py-0.5 rounded break-all">{{ parseResult.search }}</span>
            </div>
          </div>
        </div>
          ...
          <div class="overflow-x-auto max-h-[400px]">
            <table class="w-full text-sm">
              <thead class="bg-gray-50">
                <tr>
                  <th class="px-4 py-2 text-left text-gray-600 font-medium w-12 text-xs">#</th>
                  <th class="px-4 py-2 text-left text-gray-600 font-medium text-xs">参数名</th>
                  <th class="px-4 py-2 text-left text-gray-600 font-medium text-xs">参数值</th>
                  <th class="px-4 py-2 text-left text-gray-600 font-medium text-xs">编码后</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100">
                <tr v-for="(param, index) in parseResult.params" :key="index" class="hover:bg-gray-50">
                  ...
                  <td class="px-4 py-2 font-mono text-teal-700 text-xs bg-teal-50/50">{{ param.key }}</td>
                  <td class="px-4 py-2 font-mono text-gray-900 text-xs break-all">{{ param.value }}</td>
                  <td class="px-4 py-2 font-mono text-gray-500 text-xs break-all">{{ param.encoded }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div v-else-if="parseResult.search === '无'" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center text-gray-400">
          <p class="text-xs">该 URL 不包含查询参数</p>
        </div>
      </div>
    </div>

    <!-- 使用提示 -->
    <div class="mt-4 bg-teal-50 rounded-xl p-3 border border-teal-100">
      <h4 class="text-xs font-semibold text-teal-800 mb-1">💡 功能说明</h4>
      <ul class="text-[11px] text-teal-700 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 leading-relaxed">
        <li>• <strong>编码</strong>：将文本转为 URL 安全格式，支持 encodeURI 和 encodeURIComponent 模式。</li>
        <li>• <strong>解码</strong>：将编码后的 URL 字符串还原为原始文本。</li>
        <li>• <strong>URL 解析</strong>：解析 URL 的协议、主机、路径、查询参数等组成部分。</li>
        <li>• <strong>数据安全</strong>：所有处理均在浏览器本地完成，数据不会上传至服务器。</li>
      </ul>
    </div>
