...
- 使用 `pnpm`，不要使用 `npm` 或重新生成 `package-lock.json`。
- 依赖锁文件使用 `pnpm-lock.yaml`。
...
- 改动依赖、工具元数据、JSON/文件处理等共享逻辑后，至少运行 `pnpm test`。
- 涉及类型、页面结构或 Nuxt 配置时，尽量运行 `pnpm typecheck`，并在结果中说明是否存在已有类型问题。
